...
opes = data[["Id", "Workset", "Structural", "Bearing"]]
df = opes.copy()

workset = pd.get_dummies(df["Workset"])
bearing = pd.get_dummies(df["Bearing"])

df.drop(columns=["Workset", "Structural", "Bearing"], axis=1, inplace=True)

df = df.join(workset)
df = df.join(bearing)

dfx = df[["BIMMS_OPEs_Precst", "Bearing", "Non-bearing"]]

from sklearn.mixture import BayesianGaussianMixture

bgm = BayesianGaussianMixture(n_components=4, n_init=20)
bgm.fit(dfx)
clusters = bgm.predict(dfx)

outliers = df[clusters == 3]
print(outliers)
print("========== OUTLIERS IDs ==========")
print(outliers["Id"])
